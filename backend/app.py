...
def config_to_variables(config):
    ...
    # LAN 自定义放行端口规则
    lan_allowed = config.get("lan_allowed_ports", "")
    if isinstance(lan_allowed, list):
        lan_ports = [str(p).strip() for p in lan_allowed if str(p).strip()]
    else:
        lan_ports = [p.strip() for p in str(lan_allowed).replace("，", ",").split(",") if p.strip()]
    if lan_ports:
...
def save_network_config():
    ...
    if not data:
        return jsonify({"success": False, "message": "请求体不能为空"}), 400
    try:
        # PPPoE 拨号模式下，检查防火墙规则是否已应用
        if data.get('wan_pppoe', False):
            result = subprocess.run(['nft', 'list', 'set', 'inet', 'fw4', 'allowed4'],
                                    capture_output=True, text=True, timeout=5)
            if result.returncode != 0:
                return jsonify({
                    "success": False,
                    "message": "防火墙规则未生效，PPPoE 拨号成功后将无防火墙保护。请先配置好防火墙规则并点击\"应用规则\"，再\"保存网络配置\"。",
